...
# 🆕 V7.5 — Filtres anti-chute / anti-pump
CHUTE_VARIATION_24H     = -2.0   # Skip si chute > 2% sur 24h
PUMP_VARIATION_24H      = 2.0    # Skip si pump > 2% sur 24h
MA_PERIODE              = 20
...
TRAILING_NIVEAUX = [
    (100, 0.05), (75, 0.07), (50, 0.10), (35, 0.15), (25, 0.20),
    (18, 0.30), (12, 0.50), (7.5, 0.80), (3, 1.50), (0.75, 2.00), (0, 2.50),
]
...
MARCHES = ["XRPUSDT", "ATOMUSDT", "LINKUSDT", "ADAUSDT", "SOLUSDT", "AVAXUSDT", "NEARUSDT", "DOTUSDT"]
KRAKEN_SYMBOLS = {"XRPUSDT":"XXRPZUSD","ATOMUSDT":"ATOMUSD","LINKUSDT":"LINKUSD","ADAUSDT":"ADAUSD","SOLUSDT":"SOLUSD","AVAXUSDT":"AVAXUSD","NEARUSDT":"NEARUSD","DOTUSDT":"DOTUSD"}
...
def get_prix_actuel(symbole):
    ...
    try:
        r = requests.get("https://api.kraken.com/0/public/Ticker", params={"pair": ks}, timeout=10)
        data = r.json()
        if data.get("error"):
            return None
...
def get_klines(symbole, limite=100):
    ks = KRAKEN_SYMBOLS.get(symbole, symbole)
    try:
        r = requests.get("https://api.kraken.com/0/public/OHLC", params={"pair": ks, "interval": 60}, timeout=15)
        data = r.json()
        if data.get("error"):
            return None
        result = data.get("result", {})
        ...
        if not keys:
            return None
        df = pd.DataFrame(result[keys[0]], columns=['time','open','high','low','close','vwap','volume','count'])
...
def calculer_adx(df, periode=14):
    try:
        v = ADXIndicator(high=df['high'], low=df['low'], close=df['close'], window=periode).adx().iloc[-1]
        return round(float(v), 2) if not pd.isna(v) else 0
    except: return 0

def calculer_atr(df, periode=14):
    try:
        v = AverageTrueRange(high=df['high'], low=df['low'], close=df['close'], window=periode).average_true_range().iloc[-1]
        return round(float(v), 8) if not pd.isna(v) else 0
    except: return 0

def calculer_rsi(df, periode=14):
    try:
        v = RSIIndicator(close=df['close'], window=periode).rsi().iloc[-1]
        return round(float(v), 2) if not pd.isna(v) else 50
    except: return 50

def verifier_volume(df):
    vols = df['volume'].tolist()
    if len(vols) < 10: return True, 0
...
def detecter_chute_libre(df, periode=MA_PERIODE):
    if len(df) < periode: return False, 0
    closes = df['close']
    ...
    chute = (ma_actuelle < ma_ancienne) and (prix < ma_actuelle) and (variation < CHUTE_VARIATION_24H)
...
def detecter_pump_vertical(df, periode=MA_PERIODE):
    ...
    prix_24h = closes.iloc[-24] if len(closes) >= 24 else closes.iloc[0]
    variation = ((prix - prix_24h) / prix_24h) * 100
    pump = (ma_actuelle > ma_ancienne) and (prix > ma_actuelle) and (variation > PUMP_VARIATION_24H)
...
def analyser_marche(symbole):
    ...
    if adx > ADX_MAX:
        ...
        return "NEUTRE", {}
    details = {"adx": adx, "atr": atr, "rsi": rsi, "atr_pct": atr_pct, "volume_ratio": vol_ratio, "df": df}

    if rsi < RSI_ACHAT:
        chute, var = detecter_chute_libre(df)
        if chute:
            log.info(f"  {symbole} : RSI {rsi} mais CHUTE LIBRE ({var}% 24h) → 🛡️ SKIP")
        ...
        if pump:
            log.info(f"  {symbole} : RSI {rsi} mais PUMP VERTICAL (+{var}% 24h) → 🛡️ SKIP")
            return "NEUTRE", details
...
def choisir_meilleur_marche():
    ...
    for m in MARCHES:
        ...
        if d != "NEUTRE":
            signaux[m] = {"direction": d, "details": det}
        time.sleep(0.5)
    if not signaux:
    ...
    log.info(f"  => SIGNAL : {meilleur} ({d}) | RSI {signaux[meilleur]['details']['rsi']}")
...
def simuler_trade(symbole, direction, num, capital, details, etat):
    ...
    if pe is None: return "ERREUR", 0, 0, {}
    ...
    log.info(f"\n  TRADE #{num} V7.5 — {symbole} {direction} @ {pe} | Stop {sl} | Mise {mise}€")
    telegram(f"📊 <b>TRADE #{num} V7.5</b>\n{'🟢' if direction=='ACHAT' else '🔴'} {symbole}\nRSI {details.get('rsi')}\nPrix {pe} | Stop {sl}\nMise {mise}€ x{LEVIER}")
    ...
    while True:
        ...
        if not be_lock and pnl_max >= BREAK_EVEN_TRIGGER_PNL:
            if direction == "ACHAT":
                ...
                if sbe < sa: sa = sbe
            be_lock = True
            log.info(f"  🔒 BREAK-EVEN → Stop {sa} (PnL max +{pnl_max}€)")
        ...
        if direction == "ACHAT":
            if pa > mp: mp = pa
            ns = round(mp - dt, 8)
            if ns > sa: sa = ns; modif = True
            if be_lock:
                smin = round(pe * (1 + BREAK_EVEN_BUFFER_PCT), 8)
                if sa < smin: sa = smin
        else:
            if pa < mp: mp = pa
            ns = round(mp + dt, 8)
            if ns < sa: sa = ns; modif = True
            if be_lock:
                smax = round(pe * (1 - BREAK_EVEN_BUFFER_PCT), 8)
                if sa > smax: sa = smax

        if mult != na and modif:
            gprot = round(((sa - pe) if direction == "ACHAT" else (pe - sa)) / pe * mise * LEVIER, 2)
            log.info(f"  [TRAILING] PnL {pnl}€ → ATR×{mult} | Stop {sa} | Protège {gprot}€")
            na = mult
        ...
        if time.time() - dl >= 60:
            log.info(f"  [{datetime.now().strftime('%H:%M:%S')}] {symbole}: {pa} | PnL: {'+' if pnl>=0 else ''}{pnl}€ | Stop: {sa}{' 🔒' if be_lock else ''} | {d}min")
            dl = time.time()

        ti = {"prix_entree": pe, "prix_sortie": ps, "stop_loss": sl, "objectif": of, "duree_minutes": d}

        if ap:
            gp = round(pnl * 0.5, 2); pe_done = True
            log.info(f"  ⚡ SORTIE PARTIELLE 50% : +{gp}€")
...
def verifier_kill_switch(etat, capital):
    ...
    if etat["pertes_consecutives"] >= MAX_PERTES_CONSECUTIVES:
        telegram(f"⚠️ KILL SWITCH — pause 12h")
...
def afficher_dashboard(etat):
    wr = (etat["nb_wins"] / etat["nb_trades"] * 100) if etat["nb_trades"] > 0 else 0
    perf = ((etat["capital"] - CAPITAL_INITIAL) / CAPITAL_INITIAL * 100)
    log.info(f"\n  Capital {etat['capital']}€ ({'+' if perf>=0 else ''}{round(perf,2)}%) | Trades {etat['nb_trades']} | WR {wr:.1f}% | Net {etat['cumul_net']}€")
...
def demarrer_bot():
    ...
    telegram(f"🚀 <b>BOT V7.5</b>\nCapital {etat['capital']}€\n🛡️ Filtres anti-chute/pump actifs")
    while True:
        try:
            s = verifier_kill_switch(etat, etat["capital"])
            if s == "RUINE": break
            if s == "PAUSE":
                etat = charger_etat(); continue
            ...
            etat["nb_trades"] += 1
            res, gain, mise, ti = simuler_trade(sym, d, etat["nb_trades"], etat["capital"], det, etat)
            if res == "ERREUR":
                etat["nb_trades"] -= 1
                time.sleep(PAUSE); continue
            ...
            if res == "GAGNE":
                ...
                etat["pertes_consecutives"] = 0
                gp = (gain / max(mise * LEVIER, 1)) * 100
                etat["avg_win_pct"] = gp if etat["avg_win_pct"] == 0 else round((etat["avg_win_pct"] * (etat["nb_wins"]-1) + gp) / etat["nb_wins"], 4)
            else:
                etat["nb_losses"] += 1
                etat["total_perdu"] = round(etat["total_perdu"] + abs(gain), 2)
                etat["pertes_consecutives"] += 1
                pp = (abs(gain) / max(mise * LEVIER, 1)) * 100
                etat["avg_loss_pct"] = pp if etat["avg_loss_pct"] == 0 else round((etat["avg_loss_pct"] * (etat["nb_losses"]-1) + pp) / etat["nb_losses"], 4)
            enregistrer_trade({'marche':sym,'direction':d,'resultat':res,'prix_entree':ti['prix_entree'],'prix_sortie':ti['prix_sortie'],'stop_loss':ti['stop_loss'],'objectif':ti['objectif'],'mise':mise,'gain':round(gain,2),'capital_apres':etat['capital'],'duree_minutes':ti['duree_minutes'],'score':None,'adx':det.get('adx'),'atr':det.get('atr'),'rsi':det.get('rsi')})
            sauvegarder_etat(etat)
            etat['historique'].append({'heure':datetime.now().strftime('%Y-%m-%d %H:%M'),'marche':sym,'direction':d,'resultat':res,'gain':round(gain,2),'mise':round(mise,2),'capital':etat['capital']})
            afficher_dashboard(etat)
            time.sleep(PAUSE)
