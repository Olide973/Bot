...
║   Trailing Stop Progressif avec affichage Protège           ║
╚══════════════════════════════════════════════════════════════╝
"""
...
# Trailing Stop Progressif selon votre tableau (avec Protège)
TRAILING_NIVEAUX = [
    ...
    (  4, 2.50),   # seuil ajouté pour respecter la logique (mais multi inchangé)
    (  0, 2.50),   # défaut
]
    ...
    while True:
        ...
        if direction == "ACHAT":
            ...
            if nouveau_stop > stop_actuel:
                ...
                # Calcul du gain protégé : PnL au stop
                gain_protege = round((stop_actuel - prix_entree) / prix_entree * mise * LEVIER, 2)
                log.info(f"  [TRAILING] PnL {'+' if pnl>=0 else ''}{pnl}€ → ATR×{multiplicateur} | Stop : {stop_actuel} | Protège : {'+' if gain_protege>=0 else ''}{gain_protege}€")
        ...
        if time.time() - dernier_log >= 60:
            log.info(f"  [{datetime.now().strftime('%H:%M:%S')}] {symbole}: {prix_actuel} | PnL: {'+' if pnl >= 0 else ''}{pnl}EUR | Stop: {stop_actuel} | {duree}min{' | PARTIEL ✅' if partiel_execute else ''}")
