"""
╔══════════════════════════════════════════════════════════════╗
║       BOT SCALPING V3 — VERSION FINALE OPTIMISÉE            ║
║       ADX → Volatilité → RSI → MA                          ║
...
def get_klines(symbole, limite=100):
    """Bougies de 15 minutes via Kraken"""
    kraken_symbol = KRAKEN_SYMBOLS.get(symbole, symbole)
    url = "https://api.kraken.com/0/public/OHLC"
    params = {"pair": kraken_symbol, "interval": 15}  # 15 minutes
    try:
...
# ══════════════════════════════════════════════════════════════
# FILTRE 1 — ADX (Average Directional Index)
# Priorité absolue : détecte si le marché est en tendance ou range
# ══════════════════════════════════════════════════════════════
...
def calculer_adx(highs, lows, closes, periode=14):
    ...
    ADX > 25 → tendance forte → trade possible
    """
    ...
    adx = sum(dx_list[-periode:]) / periode
    return round(adx, 2)

# ══════════════════════════════════════════════════════════════
# FILTRE 2 — VOLUME
# Vérifie que le volume est suffisant pour éviter les faux signaux
# ══════════════════════════════════════════════════════════════

def verifier_volume(volumes):
    """
    Compare le volume récent avec la moyenne des dernières 24h.
    Si volume < 50% de la moyenne → marché mort → pas de trade
    """
    if len(volumes) < 10:
        return True, 0
    moyenne_24h   = sum(volumes[-96:]) / len(volumes[-96:])  # 96 bougies de 15min = 24h
    volume_recent = sum(volumes[-4:]) / 4  # Moyenne des 4 dernières bougies (1h)
    ratio = volume_recent / moyenne_24h if moyenne_24h > 0 else 0
    suffisant = ratio >= VOLUME_MINI
    return suffisant, round(ratio * 100, 1)

# ══════════════════════════════════════════════════════════════
# FILTRE 3 — RSI (Relative Strength Index)
# Timing d'entrée : détecte suracheté/survendu
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
# FILTRE 4 — MOYENNE MOBILE
# Confirmation de la direction de tendance
# ══════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════
# ANALYSE COMPLÈTE — ADX → Volatilité → RSI → MA
# ══════════════════════════════════════════════════════════════
